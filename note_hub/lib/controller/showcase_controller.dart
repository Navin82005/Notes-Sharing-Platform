...
class ShowcaseController extends GetxController {
  ...
  static tmpFetchDocumentData() async {
    print(
        "Fetching profile posts from server ${AppMetaData.backend_url}/user/documents/${HiveBoxes.userBox.get("data")!.username}...");
    try {
      Uri uri = Uri.parse(
          "${AppMetaData.backend_url}/user/documents/${HiveBoxes.userBox.get("data")!.username}");
      var response = await http.get(uri);
